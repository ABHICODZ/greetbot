...
VAD_SPEECH_THRESHOLD = 350        # tune up/down depending on your MacBook mic sensitivity
...
pygame.display.set_caption("Robo System Control Core")
clock = pygame.time.Clock()
FONT_UI = pygame.font.SysFont("Courier", 16, bold=True)

# =====================================================================
#                     MIC INPUT (sounddevice / CoreAudio)
# =====================================================================

def ensure_mic_ready():
    """
    macOS handles audio device routing itself via CoreAudio - no
    profile switching needed like Linux Bluetooth. We just confirm
    sounddevice can see a default input device.
    """
    ...
    try:
        ...
        if default_in is None or default_in < 0:
            print("\n[MIC FAULT]: No default input device found. "
                  "Check System Settings > Sound > Input, and System Settings > "
                  "Privacy & Security > Microphone (make sure Terminal/Python has access).")
            return False
        dev_info = devices[default_in]
        print(f"\n[MIC OK]: Using '{dev_info['name']}' as input device.")
        return True
...
def capture_audio_clip():
    """
    Records directly via sounddevice using an energy-based VAD:
    starts recording when speech begins, stops after a pause. No
    subprocess/pipe needed on macOS - sounddevice talks to CoreAudio
    directly.
    """
    frames = []
    triggered = False
    ...
    try:
        with sd.InputStream(samplerate=VAD_SAMPLE_RATE, channels=1, dtype="int16",
                             blocksize=VAD_CHUNK_SAMPLES) as stream:
            while True:
...
# =====================================================================
#                     TTS OUTPUT (Piper -> afplay, macOS 'say' fallback)
# =====================================================================
...
def speak_text(text):
    ...
    if os.path.exists(CONFIG["PIPER_MODEL_PATH"]):
        try:
            ...
            if gen.returncode == 0 and os.path.exists(CONFIG["TEMP_TTS_WAV_PATH"]):
                subprocess.run(["afplay", CONFIG["TEMP_TTS_WAV_PATH"]], check=True, timeout=20)
                piper_ok = True
            else:
    ...
    if not piper_ok:
        # macOS has a built-in TTS voice via the 'say' command - always
        # available, no install needed, decent fallback quality.
        try:
...
def voice_assistant_worker():
    ...
    if not mic_ready:
        BOT_STATE["status_msg"] = "MIC FAILED - check permissions"
        BOT_STATE["user_transcript"] = "Could not access mic. Check System Settings > Privacy > Microphone."
        while True:
            time.sleep(5)

    BOT_STATE["status_msg"] = "Listening Active"
    BOT_STATE["user_transcript"] = "System Ready. Speak into your Mac's mic!"
...
def get_high_res_eye_surface(emotion, side):
    ...
    if emotion == "HAPPY":
        # Draw a thick happy crescent arc pointing up
        points = []
        ...
        # Drooping ovals tilted outwards/downwards
        openness = 0.52
        brow_y_offset = 12
        brow_tilt = side * 15
    elif emotion == "SURPRISED":
        # Wide open round circular ovals
    ...
    # Draw regular/capsule/tilted eyes (NEUTRAL, SAD, SURPRISED, THINKING)
    lid_pts = []
    ...
    if eye_h > 15:
        ...
        # Double anime highlights (bright white LEDs)
...
def draw_led_grid(dest_screen, src_surf, cx, cy, grid_spacing=4):
    """
    Downsamples the 160x140 high-res surface into a 40x35 dot grid,
    then draws concentric circles for each glowing LED pixel.
    """
    ...
    for y in range(grid_h):
        for x in range(grid_w):
            ...
            if color.a > 0 and (color.r > 10 or color.g > 10 or color.b > 10):
                ...
                # Highlight dots are rendered as bright white core, others as glowing cyan
                ...
                # Outer soft glow & Inner bright LED core
